								...
								<thead>
									<tr>
										...
										<th style="text-align: center; font-size: 14px">
											Reason</th>
											<th style="font-size: 14px">Host Type</th>
										<th style="font-size: 14px">Host Transaction ID</th>
										<th style="font-size: 14px; text-align: center !important">Sales
											Slip</th>
                                        <th style="font-size: 14px">More Details</th>
										...
										<tr>
											...
											<td style="text-align: center;"><c:if
													...
													<a href="javascript:void(0)" id="openPayoutslip"
														 data-failure-reason="${fn:escapeXml(dto.failurereason)}"												
															 onclick="javascript: openDeclinedReason(
													       (this.getAttribute('data-failure-reason') == null || this.getAttribute('data-failure-reason').trim() === '') ? 'Payout transaction failed' :
													       (this.getAttribute('data-failure-reason').includes('ERROR (') || this.getAttribute('data-failure-reason').includes('Exception') || this.getAttribute('data-failure-reason').includes('exception')) ? 'Host Side Error' :
													       (this.getAttribute('data-failure-reason').includes('Invalid payee details. Account Number / Proxy Type/Value cannot be found.')) ? 'Invalid payee details' :
													       this.getAttribute('data-failure-reason'))">														
                                               ...
                                               <td style="text-align: left; font-size: 14px">${dto.hostType}</td>
                                            ...
                                            <c:if test="${dto.payoutstatus == 'Paid'}">
												<td><a href="#"
													onclick="showAlert(event ,'${dto.invoiceidproof}')"> <img
														class="w24"
														src="${pageContext.request.contextPath}/resourcesNew1/assets/send-blue.svg" />
												</a></td>
											</c:if>
                                        --%>
										</tr>
